use retrocpu_emu_rs::cpuboard::{Tms9995Bus, Tms9995Ram};
...
/// Intel HEX のデータレコードだけを RAM へ上書きする（HEX 未指定領域は維持）。
pub fn load_intel_hex_overlay(hex_text: &str, ram: &mut Tms9995Ram) -> Result<(), FrameworkError> {
    let len = ram.len_bytes();
    load_intel_hex_overlay_bytes(hex_text, len, |addr, value| {
        if addr >= len {
            return Err(FrameworkError::invalid_argument(format!(
                "memory write out of range 0x{addr:x}"
            )));
        }
        ram.write_byte(addr as u16, value);
        Ok(())
    })
}

fn load_intel_hex_overlay_bytes(
    hex_text: &str,
    memory_len: usize,
    mut write_byte: impl FnMut(usize, u8) -> Result<(), FrameworkError>,
        ...
        match record_type {
            0x00 => {
                ...
                for (i, b) in data.iter().enumerate() {
                    ...
                    if a >= memory_len {
                        return Err(FrameworkError::invalid_argument(format!(
                            "line {}: memory write out of range 0x{a:x}",
                            line_no + 1
                        )));
                    }
                    write_byte(a, *b)?;
