...
describe("MN1613 命令: CPYB / SETB（ベースレジスタ⇔汎用レジスタ）", () => {
  ...
  test("CPYB R1, TSR1   rd=1, bbb=3: 0x0FB1", () =>
    assert.equal(asm1("        CPYB R1, TSR1"), 0x0fb1));
  ...
  test("SETB R1, TSR1   rs=1, bbb=3: 0x0F31", () =>
    assert.equal(asm1("        SETB R1, TSR1"), 0x0f31));
