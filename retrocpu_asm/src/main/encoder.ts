...
function parseEA(
  ...
  // (*D)(Xn) — 間接インデックス（sdas 風）
  m = t.match(/^\(\s*\*(.+)\s*\)\s*\(\s*(X0|X1)\s*\)$/i);
  if (m) {
    return {
      mmm: m[2].toUpperCase() === "X0" ? 0b110 : 0b111,
      d8: u8(evalExpr(m[1], symbols, allowUndefined), "EA"),
    };
  }
  // [*D], Xn — 間接インデックス（互換）
  m = t.match(/^\[\s*\*(.+)\s*\]\s*,\s*(X0|X1)$/i);
  if (m) {
    return {
      mmm: m[2].toUpperCase() === "X0" ? 0b110 : 0b111,
      d8: u8(evalExpr(m[1], symbols, allowUndefined), "EA"),
  ...
  m = t.match(/^(.+)\(\s*(X0|X1)\s*\)$/i);
  if (m) {
    return {
      mmm: m[2].toUpperCase() === "X0" ? 0b100 : 0b101,
      d8: u8(evalExpr(m[1], symbols, allowUndefined), "EA"),
    };
  }
  // D, Xn — 直接インデックス（互換）
  m = t.match(/^(.+)\s*,\s*(X0|X1)$/i);
  if (m) {
    return {
      mmm: m[2].toUpperCase() === "X0" ? 0b100 : 0b101,
      d8: u8(evalExpr(m[1], symbols, allowUndefined), "EA"),
