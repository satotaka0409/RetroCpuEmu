...
fn call_led(
    ...
    let worker = std::thread::spawn(move || serve_cpu_to_io(&io, 16));
...
fn led_seven_seg_can_clear_or_keep_bullet() -> Result<(), FrameworkError> {
    with_case(|s, mock| {
        ...
        assert_eq!(seven0[0], 0);
        assert_eq!(&seven0[1..], &SEVEN_SEG_ALT[1..]);
        assert_eq!(b00, 0);
        assert_eq!(b01, 0);
...
fn led_seven_seg_can_keep_bullet_when_r1_is_one() -> Result<(), FrameworkError> {
    with_case(|s, mock| {
        ...
        assert_eq!(seven1[0], 0);
        assert_eq!(&seven1[1..], &SEVEN_SEG_ALT[1..]);
        assert_eq!(b10, 0x55);
        assert_eq!(b11, 0xaa);
        Ok(())
    })
