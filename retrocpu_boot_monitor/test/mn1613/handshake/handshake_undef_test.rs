...
fn call_undef_led(
    ...
    let worker = std::thread::spawn(move || serve_cpu_to_io(&io, 60));
...
fn undef_notify_arg_1_returns_ok() -> Result<(), FrameworkError> {
    with_case(|s, mock| {
        let req = call_undef_led(s, &mock, 1)?;
        assert_eq!(req.len(), 60);
        assert_eq!(req[0], 0x13);
        s.expect_registers(
            &CallRegisters {
                r0: Some(0),
                ..Default::default()
...
fn undef_notify_arg_0_returns_ok() -> Result<(), FrameworkError> {
    with_case(|s, mock| {
        let req = call_undef_led(s, &mock, 0)?;
        assert_eq!(req.len(), 60);
        assert_eq!(req[0], 0x13);
        s.expect_registers(
            &CallRegisters {
...
fn undef_notify_arg_3_returns_ok() -> Result<(), FrameworkError> {
    with_case(|s, mock| {
        let req = call_undef_led(s, &mock, 3)?;
        assert_eq!(req.len(), 60);
        assert_eq!(req[0], 0x13);
        s.expect_registers(
            &CallRegisters {
