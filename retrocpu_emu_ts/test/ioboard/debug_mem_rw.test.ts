/**
 * デバッグ TCP 13h/14h → ハンドシェイク 13h/14h のメモリ R/W。
 * 根拠: HandShake.mdc / retrocpu_debug.mdc（ダンプ読み書きは DMA ではない）
 *
 * CPU 側は BIOS 結合と同じく REQ_1 後に `g_handshake_interrupt_handler` を `run()` する。
 */
...
/** ユーザ RAM（ワード）。ダンプの典型アドレス */
const WORD_ADDR = 0x1800;

/** 同じ位置のバイトアドレス（1 ワード = 2 バイト） */
const BYTE_ADDR = WORD_ADDR * 2;

/**
 * ブートモニタ IHX パス。無ければテストをスキップする。
 * @returns パス。無ければ null
 */
...
describe("デバッグ TCP メモリダンプ 13h/14h（ハンドシェイク中継）", () => {
  ...
  /**
   * モニタを DMA ロードして HALT まで進め、DebugHost を実ハンドシェイクに繋ぐ。
  ...
  it("13h は CPU RAM をハンドシェイク経由で返す", async () => {
