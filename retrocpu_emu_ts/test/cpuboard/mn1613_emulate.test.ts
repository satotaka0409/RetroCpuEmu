...
describe("reset", () => {
  ...
  it("リセット直後のクロックは 0（ベクタ IO 読みは数えない）", () => {
    expect(getClockCount()).toBe(0n);
  });
});
...
describe("H 命令", () => {
  ...
  it("H 停止中でもマスク有効な IRQ で ISR に入る", () => {
    const words: number[] = new Array(0x120).fill(0);
    words[0] = 0x2000; // H
    words[0x104] = 0x0000; // レベル2 NPSW STR（ネスト無し）
    words[0x105] = 0x0110; // レベル2 NPSW IC
    words[0x110] = 0x7807;
    words[0x111] = 0xcafe; // MVWI R0, 0xCAFE
    words[0x112] = 0x2006; // LPSW 2
    loadWords(words);
    setState({ STR: STR_M2, IC: 0 });
    step(); // H → halted
    expect(getExecStatus()).toBe("halted");
    triggerInterrupt(2);
    step(); // 起床して ISR の MVWI を実行
...
describe("未定義命令", () => {
  /**
   * NPP=1（リセット既定）向けにレベル0 NPSW と簡易 ISR を置く。
   * @param undefWord アドレス0に置く未定義命令語
   * @param opts.isrIc ISR 先頭（既定 0x0110）。そこに H を置く
   * @param opts.npswStr NPSW STR（既定 0）
   */
  function loadUndefWithLevel0Isr(
    undefWord: number,
    opts: { isrIc?: number; npswStr?: number } = {},
  ): void {
    const isrIc = opts.isrIc ?? 0x0110;
    const words: number[] = new Array(0x120).fill(0);
    words[0] = undefWord & 0xffff;
    words[0x100] = opts.npswStr ?? 0; // NPSW STR
    words[0x101] = isrIc; // NPSW IC
    words[isrIc] = 0x2000; // ISR: H
    loadWords(words);
  }
  ...
  it("レベル0 OPSW 退避と NPSW ロードが走る（M0 オフでも）", () => {
    // リセット直後 STR=0（M0 オフ）。外部 IRQ0 は受け付けないが、未定義は内部トラップで即受理。
    loadUndefWithLevel0Isr(0x0000, { isrIc: 0x0110 });
    ...
    // OPSW: アドレス0=旧STR、1=未定義実行後の IC（次語＝1）
    const view = new DataView(getMemory());
    expect(view.getUint16(0 * 2, false)).toBe(0); // 旧 STR
    expect(view.getUint16(1 * 2, false)).toBe(1); // 旧 IC（フェッチ後）
    expect(getExecStatus()).not.toBe("halted");
  });

  it("ISR から LPSW 0 で戻り、未定義の次命令へ復帰できる", () => {
    // OPSW は 0/1 を使うので、未定義は 0x20 に置く
    // 0x20: undef / 0x21: MVWI R1,0xBEEF / 0x23: H
    // ISR: MVWI R0,0xCAFE; LPSW 0
    const words: number[] = new Array(0x120).fill(0);
    words[0x20] = 0x0000; // undef
    words[0x21] = 0x7907;
    words[0x22] = 0xbeef; // MVWI R1, 0xBEEF
    words[0x23] = 0x2000; // H
    words[0x100] = 0x0000; // NPSW STR
    words[0x101] = 0x0110;
    words[0x110] = 0x7807;
    words[0x111] = 0xcafe; // MVWI R0, 0xCAFE
    words[0x112] = 0x2004; // LPSW 0
    ...
    expect(getState().R[0]).toBe(0xcafe);
    step(); // MVWI R1
    expect(getState().R[1]).toBe(0xbeef);
  });
  ...
  it("H（0x2000）は halted だが IISR bit15 は立てない", () => {
    loadWords([0x2000]);
    step();
    expect(getExecStatus()).toBe("halted");
    expect(getState().IISR & IISR_UNDEF).toBe(0);
  ...
  it("M0 有効時も未定義はレベル0へ入り、外部 IRQ0 と同じ OPSW スロットを使う", () => {
    loadUndefWithLevel0Isr(0x0000);
    setState({ STR: STR_M0 });
    step();
    const view = new DataView(getMemory());
...
// ─────────────────────────────────────────────
// リセットからの CPU クロック（MN1613.mdc: 1 アクセス = 4 clk）
// ─────────────────────────────────────────────
describe("clock count", () => {
  it("1語命令（MVI + H）はフェッチ 2 回 = 8 clk", async () => {
    await runHalt([0x0800, 0x2000]); // MVI R0, #0 / H
    expect(getClockCount()).toBe(BigInt(CPU_CLK_PER_ACCESS * 2));
  });

  it("2語命令（MVWI + H）はフェッチ 3 回 = 12 clk", async () => {
    await runHalt([0x7807, 0x1234, 0x2000]); // MVWI R0, #0x1234 / H
    expect(getClockCount()).toBe(BigInt(CPU_CLK_PER_ACCESS * 3));
  });

  it("L ZP はフェッチ + データ読み = 8 clk（+ H で 12）", async () => {
    await runHalt([0xc000, 0x2000]); // L R0, *0 / H
    expect(getClockCount()).toBe(BigInt(CPU_CLK_PER_ACCESS * 3));
  });

  it("RD はフェッチ + IO 読み = 8 clk（+ H で 12）", async () => {
    await runHalt([0x1801, 0x2000]); // RD R0, #1 / H
    expect(getClockCount()).toBe(BigInt(CPU_CLK_PER_ACCESS * 3));
  });

  it("HALT 中の tickCpu はクロックを進めない", async () => {
    await runHalt([0x2000]);
    const afterH = getClockCount();
    expect(afterH).toBe(BigInt(CPU_CLK_PER_ACCESS));
    tickCpu();
    tickCpu();
    expect(getClockCount()).toBe(afterH);
  });

  it("reset するとクロックが 0 に戻る", async () => {
    await runHalt([0x0800, 0x2000]);
    expect(getClockCount()).toBeGreaterThan(0n);
    reset();
    expect(getClockCount()).toBe(0n);
  });

  it("powerOnIdle でもクロックは 0", () => {
    powerOnIdle();
    expect(getClockCount()).toBe(0n);
