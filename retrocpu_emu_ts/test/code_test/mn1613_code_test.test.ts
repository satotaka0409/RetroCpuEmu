...
describe("intel hex + cdb", () => {
  ...
  it("__CP チェックポイントは checkpoints に保存（byName / symbols に混ぜない）", () => {
    const cdb = parseCdb(
      "L:G$add$0$0:3000\nL:__CP$uart_initialized$0001:3000\n",
    );
    expect(cdb.byName.get("add")?.wordAddr).toBe(0x1800);
    expect(cdb.byName.get("uart_initialized")).toBeUndefined();
    expect(cdb.symbols.some((s) => s.scope === "__CP")).toBe(false);
    expect(cdb.checkpoints).toEqual([
      {
  ...
  it("不正な __CP レコードはエラー", () => {
    expect(() => parseCdb("L:__CP$bad:3000\n")).toThrow(/Invalid checkpoint/);
  });
});
...
describe("Mn1613CodeTest.call", () => {
  it("R0=R0+R1 を呼び出し R0 を検証する", async () => {
    ...
    t.loadIntelHex(hex);
    t.loadCdb(`${cdb}L:__CP$uart_initialized$0001:3000\n`);
    expect(t.getCheckpoints()).toEqual([
      {
        id: "__CP$uart_initialized$0001",
        name: "uart_initialized",
        serial: "0001",
