...
export class AsmDiagnostics {
	...
	refresh(document: vscode.TextDocument): void {
		...
		for (const hit of findInvalidCpuDirectives(
			document.getText(),
		)) {
		...
		for (let lineNo = 0; lineNo < document.lineCount; lineNo += 1) {
			...
			if (parseGlobalDirectiveNames(line.text) !== null) {
				...
				for (const hit of findUnusedGlobalDeclarations(
					...
					arch,
					(n) =>
						this.index.hasOperandReference(
							n,
						) ||
						this.index.hasLabelOrEqu(n),
				)) {
					const d = new vscode.Diagnostic(
						new vscode.Range(
							lineNo,
			...
			for (const hit of [
				...
				...findImmRangeOverflows(
					line.text,
					parsed,
					arch,
				),
				...findTms9995SyntaxIssues(
					line.text,
					parsed,
					arch,
				),
	...
	refreshOpen(): void {
		for (const doc of vscode.workspace.textDocuments) {
			if (isAsmLanguageId(doc.languageId)) this.refresh(doc);
		}
	}
}
