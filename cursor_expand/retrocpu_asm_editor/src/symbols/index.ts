...
export class SymbolIndex {
	...
	hasLabelOrEqu(name: string): boolean {
		return this.lookup(name).some(
			(s) => s.kind === "label" || s.kind === "equ",
		);
	}
	...
	async rebuild(): Promise<number> {
		...
		for (const uri of uris) {
			try {
				...
				const found = this.collectSymbolsFromText(
					uriKey,
					text,
					addTemp,
				);
	...
	private walkIncludes(
		...
		for (const incPath of collectIncludePaths(text)) {
			...
			nextRefs.set(
				...
				collectOperandRefCounts(
					incText,
					detectArchitecture(abs, incText),
				),
			);
			this.walkIncludes(
				abs,
	...
	private removeUri(uriKey: string): void {
		...
		for (const sym of old) {
			...
			const next = list.filter(
				(s) => s.uri !== uriKey || s.line !== sym.line,
			);
