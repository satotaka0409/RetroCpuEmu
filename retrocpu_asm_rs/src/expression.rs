//! 即値・ラベル式の評価（sdas 互換の `#` / 基数サフィックス / 文字リテラル）。

use std::collections::HashMap;

use crate::error::AsmError;

/// 数値トークンを i32 に解釈（`0x` / `>hex` / `123H` / 10 進など）。
...
/// 式を評価する（演算子優先度: 単項 → */% → +- → <<>> → & → ^ → |）。
///
/// * `allow_undefined` — 第 1 パス（サイズ計算）では未定義シンボルを 0 扱い。
pub fn eval_expr(
	expr: &str,
...
/// `.word "AB"` など二重引用符文字列を ASCII コード列へ展開する。
///
/// 文字列リテラルでなければ None。
