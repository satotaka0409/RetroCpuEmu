//! CPU 種別の解決（`.cpu` ディレクティブ / CLI）。

use crate::error::AsmError;
use crate::parser::strip_line_comment;

/// サポート CPU（`.cpu` / `--cpu` で指定）。
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum CpuType {
	/// Panasonic MN1613（ワードアドレス）。
	Mn1613,
	/// TMS9995（バイトアドレス。命令エンコードはプレースホルダ）。
	Tms9995,
}

/// 文字列を CPU 種別へ変換する。未知は None。
...
/// ソース先頭付近の `.cpu` を走査する（最初の非コメント行のみ許可）。
