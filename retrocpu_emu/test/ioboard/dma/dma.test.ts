...
import {
  ...
  liveDmaCpuBridge,
  type DmaCpuBridge,
} from "../../../src/ioboard/dma/dma";
import { CpuDmaTarget, isCpuDmaBusy } from "../../../src/cpuboard/cpu_dma";
import {
  getPins,
  reset,
  setMemory,
...
describe("DmaMaster (write-only)", () => {
  ...
  it("readBytes は持たない", () => {
    expect("readBytes" in master).toBe(false);
    expect(typeof master.writeBytes).toBe("function");
  });
  ...
  it("isRunning が true のままならタイムアウトする", async () => {
    ...
    const bus = createDmaBus();
    const m = new DmaMaster(bus, dmaWriteMemoryFromArrayBuffer(buf), 50, stuck);
...
describe("CpuDmaTarget (write-only, HALT/RESET)", () => {
  ...
  it("readBytes / readWords は持たない", () => {
    const dma = new CpuDmaTarget(200);
    expect("readBytes" in dma).toBe(false);
    expect("readWords" in dma).toBe(false);
    expect(typeof dma.writeBytes).toBe("function");
    expect(typeof dma.writeWords).toBe("function");
  });
