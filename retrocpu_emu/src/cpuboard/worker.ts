 ...
 * 起動は idle。ブートモニタの DMA と RST は IO ボード（F7 / 電源投入）が行う。
 */
...
function publishStatus(): void {
  ...
  const clock = clockCountToHiLo(getClockCount());
  Atomics.store(s, STATUS.CLOCK_LO, clock.lo);
  Atomics.store(s, STATUS.CLOCK_HI, clock.hi);
}
