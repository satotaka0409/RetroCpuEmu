 ...
 * 根拠: HandShake.mdc「タイマー設定」(19h) / ioboard.mdc
 *
 * リセット直後は停止状態。CPU ボードからハンドシェイク 19h で
 * 周期 (ms) と回数を受け取ったときだけ動き出す。周期 0 または
 * stop() で停止する。満了ごとに onExpire を呼ぶだけで、
 * レベル 2 割り込みの実配送は呼び出し側（IO ボード結線側）の責務。
 */

import { RESPONSE_CODE } from "../cpu/mn1613/handhshake/handshake_type";

/**
 * タイマー 1 本の設定値。
 * ハンドシェイク 19h の TimerParams からタイマー番号を除いた形。
 */
