...
function _handleIRQ(): void {
  for (let lv = 0; lv <= 2; lv++) {
    ...
    if (_pendingIRQ & (1 << lv) && cpuRegister.STR & mask) {
      _pendingIRQ &= ~(1 << lv);
      cpuRegister.OSR[lv] = cpuRegister.CSBR & 0xf; // CSBR を OSR に退避
      cpuRegister.CSBR = 0; // CSBR を 0 クリア
      // OPSW を物理アドレス 0 ベースに保存
      _wrPhys(_phys(lv * 2, 0), cpuRegister.STR);
      _wrPhys(_phys(lv * 2 + 1, 0), cpuRegister.IC);
      // NPSW をロード（NPP << 8 が NPSW 領域先頭）
...
function _executeOne(): void {
  ...
  switch (op) {
    ...
    default:
      _execStatus = "halted";
      cpuRegister.IISR |= 0x8000; // MN1613: 未定義命令で IISR bit15 セット
      break;
  }
}
