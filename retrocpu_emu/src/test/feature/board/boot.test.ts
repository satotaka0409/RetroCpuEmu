/**
 * コールドブート（HALT スタブ）試験
 * 根拠: MN1613.mdc リセット / board/boot.ts
 */
...
describe("MN1613 reset vector (IO:0)", () => {
  it("reset() は IO:0 を IC に載せ running になる", () => {
    setIoReadCallback((p) => (p === 0 ? 0x0200 : 0));
    reset();
    expect(getState().IC).toBe(0x0200);
...
describe("coldBootHaltStub", () => {
  ...
  it("setResetVector + attach 後の RST でベクタが効く", () => {
    attachIoBoardPorts();
    setResetVector(0x0200);
    const view = new DataView(getMemory());
    view.setUint16(0x0200 * 2, OPCODE_H, false);
    setPins({ RST: true });
    setPins({ RST: false });
    expect(getState().IC).toBe(0x0200);
    tickCpu();
